      ...
      <div>
        ...
        <LabeledSwitch label-on="Use selection" label-off="Use all" :disabled="nodeModel==null">
          <template v-slot:tooltip>
            <div> If disabled, uses all current nodes of the selected type as seeds. If enabled, only nodes from the users selection are used.</div>
          </template>
        </LabeledSwitch>
        <div >
          <v-btn style="margin: 8px" small outlined :disabled="nodeModel==null" color="primary" @click="$emit('openAlgorithmDialogEvent')">Module Identification</v-btn>
          <v-btn style="margin: 8px" small outlined :disabled="nodeModel==null" color="primary">Drug Ranking</v-btn>
        </div>
      </div>
<!--      <v-row>-->
<!--        <v-col cols="6">-->
