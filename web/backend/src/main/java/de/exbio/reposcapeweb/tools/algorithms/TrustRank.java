...
@Service
public class TrustRank implements Algorithm {
    ...
    private Logger log = LoggerFactory.getLogger(TrustRank.class);

    @Autowired
    public TrustRank(Environment env, AlgorithmUtils utils, NodeController nodeController) {
        this.env = env;
        this.utils = utils;
    ...
    public ToolService.Tool getEnum() {
        return ToolService.Tool.TRUSTRANK;
    }

    @Override
    public void prepare() {
        log.info("Setting up TrustRank");
        executable = new File(env.getProperty("path.tool.trustrank"));
        log.info("TrustRank path=" + executable.getAbsolutePath());
