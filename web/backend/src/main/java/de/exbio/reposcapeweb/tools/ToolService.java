...
//            ProcessUtils.executeProcessWait(pb,true);
    ...
    public void createLayout(File graph, File layout, String layout_type) {
        layout.getParentFile().mkdirs();
        try {
            ProcessUtils.executeProcessWait(new ProcessBuilder(getLayoutingPythonPath(), new File(layoutDir, "cartographs.py").getAbsolutePath(), graph.getAbsolutePath(), layout.getAbsolutePath(), layout_type), false);
        } catch (IOException | InterruptedException e) {
            e.printStackTrace();
        }
    }
