...
//            ProcessUtils.executeProcessWait(pb,true);
    ...
    public void createLayout(File graph, File layout, File nodes, File edges, String layout_type) {
        layout.getParentFile().mkdirs();
        try {
            ProcessUtils.executeProcessWait(new ProcessBuilder(getLayoutingPythonPath(), new File(layoutDir, "cartographs_v1.py").getAbsolutePath(), graph.getAbsolutePath(), nodes.getAbsolutePath(), edges.getAbsolutePath(), layout.getAbsolutePath(), layout_type), false);
        } catch (IOException | InterruptedException e) {
            e.printStackTrace();
        }
    }
