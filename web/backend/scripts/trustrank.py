...
def print_usage():
    print(' ')
    print('        usage: python3 trustrank.py network_file seed_file damping_factor(optional) outfile_name(optional) only_direct_drugs only_approved_drugs')
    print('        -----------------------------------------------------------------')
    ...
    print('        damping_factor     : A float value between 0-1 to be used as damping factor parameter.')
    print('                             By default the value is set to 0.85')
...
def check_input_style(input_list):
    ...
    damping_factor = 0.85
    outfile_name = 'trustrank_dampfactor_%.2f.txt' % (damping_factor)
    only_direct_drugs = True
    only_approved_drugs = True

    if len(input_list) == 6:
        try:
            damping_factor = float(input_list[3])
            outfile_name = 'trustrank_dampfactor_%.2f.txt' % (damping_factor)
    ...
    if len(input_list) == 7:
        try:
            ...
            sys.exit(0)
            return
    return network_file, seeds_file, damping_factor, outfile_name, only_direct_drugs, only_approved_drugs
...
def read_input(network_file, seed_file, only_direct_drugs, only_approved_drugs):
    ...
    * The network must be provided as a graphml or gt format file. Nodes and edges
    should have attribute type. Valid node types are: Protein, Drug. Valid edge types
    are: DrugHasTarget, ProteinInteractsWithProtein.
...
# =============================================================================
def trustrank(G_lcc, seed_ids, drug_ids, damping_factor, outfile=None):
    """
    ...
    ------
     ...
     - outfile:
              ...
              if not given the program will name it 'trustrank_dampfactor_X.txt'
     - damping_factor:
                      a float value between 0-1 to be used as damping factor parameter
     Returns:
     --------
    ...
    trust = G_lcc.new_vertex_property("double")
    trust.a[seed_ids] = 1.0 / len(seed_ids)
    # damping_factor = 0.5
    scores = gtc.pagerank(G_lcc, damping=damping_factor, pers=trust, weight=weights)
...
if __name__ == '__main__':
    ...
    #     used only)
    #
    # [3] (optional) float value between 0-1 to be used as damping factor parameter
    #
    # [4] (optional) name for the results file
    #
    # [5] Letter "Y" to include only direct drugs in ranking or letter "N"
    #     to include all the drugs
    #
    # [6] Letter "Y" to include only approved drugs in ranking or letter "N"
    #     to include all the drugs

    # check if input style is correct
    input_list = sys.argv
    network_file, seeds_file, damping_factor, outfile_name, only_direct_drugs, only_approved_drugs = check_input_style(input_list)
    ...
    # run closeness
    drug_scores = trustrank(G_lcc, seed_ids, drug_ids, damping_factor, outfile=outfile_name)
