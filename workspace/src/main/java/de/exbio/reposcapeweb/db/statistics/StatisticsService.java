...
public class StatisticsService {
    ...
    public TreeMap<Integer, Integer> getDrugHasIndicationOut() {
        TreeMap<Integer, Integer> result = new TreeMap<>();
        statisticsRepository.get_drug_has_target_out().forEach(d -> result.put(d.getDegree(), d.getCount()));
        return result;
    }

    public TreeMap<Integer, Integer> getDrugHasIndicationIn() {
        TreeMap<Integer, Integer> result = new TreeMap<>();
        statisticsRepository.get_drug_has_target_in().forEach(d -> result.put(d.getDegree(), d.getCount()));
        return result;
    }


    public TreeMap<Integer, Integer> getDrugHasTargetOut() {
        TreeMap<Integer, Integer> result = new TreeMap<>();
        statisticsRepository.get_drug_has_target_out().forEach(d -> result.put(d.getDegree(), d.getCount()));
        return result;
    }

    public TreeMap<Integer, Integer> getDrugHasTargetIn() {
        TreeMap<Integer, Integer> result = new TreeMap<>();
        statisticsRepository.get_drug_has_target_in().forEach(d -> result.put(d.getDegree(), d.getCount()));
        return result;
    }
